...
/** namespace planar_beam contains code to solve a planar stress problem.
 */
namespace planar_beam{
template <int dim>
/** implementing 2D FEM computation of a beam structure
* plane stress assumption, two dimensional entity element is used.
*/
class Beam
{
...
private:
	...
	static const double LamesFirstParameter;//!< for plane stress case, the formula is \f$ \frac{E\mu}{1-\mu^2};\f$
...
void Beam<dim>::run()
{
	read_grid("beam_triangulation.msh");
	//test_boundary();
...
void Beam<dim>::assemble_system()
{
	...
	const post_processing::RightHandSide<dim> right_hand_side;
...
int main(int argc)
{
	planar_beam::Beam<2> my_planar_beam;
	my_planar_beam.run();
	return 0;
}
